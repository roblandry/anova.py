...
logging.basicConfig(
    filename="/tmp/anova_control.log",
    filemode="a",
...
def check_arg(args=None):
    ...
    parser.add_argument("-i", "--cooker_id", type=str, help="Cooker Id.", required="True")
    parser.add_argument("-e", "--secret", type=str, help="Cooker Secret.", required="True")
    parser.add_argument(
        "-m",
        "--method",
        type=str,
    ...
    parser.add_argument(
        "-t", "--temperature", type=int, help='Set temperature (required when using "start")'
    )
    parser.add_argument(
        "-d",
        "--duration",
...
def main():
    ...
    i, e, m, t, d, u, s, o = check_arg(sys.argv[1:])
    ...
    cooker = danodemano_anovapy.AnovaCooker(i, e)
